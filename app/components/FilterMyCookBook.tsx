...
const Filter = ({
  ...
  const updateUrl = (dishTypeId: number, ingredientIds: number[], page: number) => {
    ...
    // Оновлення URL
    router.push(`/dashboard/cookbook?${queryParams.toString()}`);
  };
  ...
  useEffect(() => {
    ...
    if (pathname === '/dashboard/cookbook' && !dishTypeParam && !ingredientsParam && !pageParam) {
  ...
  const resetFilters = () => {
    ...
    router.push('/dashboard/cookbook'); // Перехід на /recipe без параметрів
  };
